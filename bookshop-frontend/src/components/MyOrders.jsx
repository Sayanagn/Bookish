...
function MyOrders(){
    ...
    useEffect(()=>{
        axios.get("http://localhost:8081/api/orders?custid="+sessionStorage.getItem("id"))
        .then(resp=>{
                    ...
                    <table className="table table-bordered table-sm table-light table-striped">
                        ...
                        <tbody>
                            {details.map(x => (
                                <tr>
                                    <td>{x.book.bookid}</td>
                                    <td><img className="mr-2 float-left" src={"http://localhost:8081/"+x.book.photo} width="100" />
                                    {x.book.bname}<br/>
                                    Category: {x.book.cat}<br/>
                                    Author: {x.book.author}<br/>
                                    </td>
                                    <td>{x.book.price}</td>
                                    <td>{x.qty}</td>
                                </tr>
