...
function AllProduct(){
    ...
    useEffect(()=>{
        ...
        if(pcat.cat!==undefined){
            axios.get("http://localhost:8081/api/books/cats?cat="+pcat.cat)
            .then(resp=>{
    ...
    const addToCart=item=>{  
        if(sessionStorage.getItem("userid")==null){
            ...
            if(checkItem(item.bookid) && checkItem(item.qty>=0))
            {     
                showModal() 
                setDisplay("none")
                setShowDialog("modal fade") 
                    ...
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <div className="d-flex">
                                <img src={"http://localhost:8081/"+item.photo} style={{width:"200px"}}/>
                                <div className="ml-3">
                                    ...
                                    <p className="font-weight-bold">Available Quantity: {item.qty}</p>
                                    <p className="font-weight-bold">Price: &#8377; {item.price}</p>
                                    <div className="form-group form-row">
                                        <label className="col-sm-5 col-form-label font-weight-bold">Quantity</label>
                                        <div className="col-sm-7">
                                    <input type="number" className="form-control" max={item.qty} min="1" value={qty} onChange={e=>setQty(e.target.value)}/>
                                    </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="modal-footer">
                            <button onClick={e=>addToCart(item)} className="btn btn-success btn-sm">Add to Cart</button>
                        </div>
                    </div>
                </div>
            </div>) : ""}
